...
public class AlienDictionary {
    ...
    private static String alienOrder(String[] words) {
        ...
        //Create all the DS for the solution
        Map<Character, List<Character>> adj = new HashMap<>();
        Map<Character, Integer> counts = new HashMap<>();

        for (String word : words) {
            for (int i = 0; i < word.length(); i++) {
                //collect all the unique characters in the input
                adj.put(word.charAt(i), new ArrayList<>());
                counts.put(word.charAt(i), 0);
            }
        }

        //Traverse the dict. Build the dependency graph
